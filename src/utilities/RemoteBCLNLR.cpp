...
#include <openstudio/utilities/core/UnzipFile.hpp>
#include <openstudio/utilities/core/DeprecatedHelpers.hpp>

#include <regex>

#define REMOTE_PRODUCTION_SERVER "https://bcl.nlr.gov"
#define REMOTE_DEVELOPMENT_SERVER "https://bcl2.nlr.gov"
...
web::http::client::http_client RemoteBCLNLR::getClient(const std::string& url, unsigned timeOutSeconds) {
  web::http::client::http_client_config config;
  // bcl can be slow to respond to client requests so bump the default of 30 seconds to 60 to account for lengthy response time.
...
unsigned RemoteBCLNLR::timeOutSeconds() const {
  return m_timeOutSeconds;
}
bool RemoteBCLNLR::setTimeOutSeconds(unsigned timeOutSeconds) {
...
bool RemoteBCLNLR::DownloadFile::open() {
  OS_ASSERT(!m_fileName.empty());
  m_ofs.open(toString(m_fileName).c_str(), std::ios_base::trunc | std::ios_base::out | std::ios_base::binary);
  return m_ofs.good();
}

RemoteBCLNLR::DownloadFile::DownloadFile(openstudio::path t_path) : m_fileName(std::move(t_path)) {}

void RemoteBCLNLR::DownloadFile::flush() {
  if (m_ofs.good()) {
    m_ofs.flush();
  }
}

const openstudio::path& RemoteBCLNLR::DownloadFile::fileName() const noexcept {
  return m_fileName;
}

void RemoteBCLNLR::DownloadFile::close() {
  m_ofs.close();
}

void RemoteBCLNLR::DownloadFile::write(const std::vector<unsigned char>& data) {
  m_ofs.write(reinterpret_cast<const char*>(data.data()), data.size());
}

RemoteBCLNLR::RemoteBCLNLR()
...
boost::optional<BCLComponent> RemoteBCLNLR::getComponent(const std::string& uid, const std::string& /*versionId*/) const {
  bool downloadStarted = const_cast<RemoteBCLNLR*>(this)->downloadComponent(uid);
  if (downloadStarted) {
...
boost::optional<BCLMeasure> RemoteBCLNLR::getMeasure(const std::string& uid, const std::string& /*versionId*/) const {
  bool downloadStarted = const_cast<RemoteBCLNLR*>(this)->downloadMeasure(uid);
  if (downloadStarted) {
...
std::vector<BCLSearchResult> RemoteBCLNLR::searchComponentLibrary(const std::string& searchTerm, const std::string& componentType,
  ...
  bool searchStarted = const_cast<RemoteBCLNLR*>(this)->startComponentLibrarySearch(searchTerm, componentType, "nrel_component", page);
  if (searchStarted) {
...
std::vector<BCLSearchResult> RemoteBCLNLR::searchComponentLibrary(const std::string& searchTerm, const unsigned componentTypeTID,
  ...
  bool searchStarted = const_cast<RemoteBCLNLR*>(this)->startComponentLibrarySearch(searchTerm, componentTypeTID, "nrel_component", page);
  if (searchStarted) {
    return waitForSearch();
  }
  return {};
...
std::vector<BCLSearchResult> RemoteBCLNLR::searchMeasureLibrary(const std::string& searchTerm, const std::string& componentType,
  ...
  bool searchStarted = const_cast<RemoteBCLNLR*>(this)->startComponentLibrarySearch(searchTerm, componentType, "nrel_measure", page);
  if (searchStarted) {
    return waitForSearch();
  }
  return {};
...
std::vector<BCLSearchResult> RemoteBCLNLR::searchMeasureLibrary(const std::string& searchTerm, const unsigned componentTypeTID,
  ...
  bool searchStarted = const_cast<RemoteBCLNLR*>(this)->startComponentLibrarySearch(searchTerm, componentTypeTID, "nrel_measure", page);
  if (searchStarted) {
    return waitForSearch();
  }
  return {};
...
int RemoteBCLNLR::checkForComponentUpdates() {
...
int RemoteBCLNLR::checkForMeasureUpdates() {
...
std::vector<BCLSearchResult> RemoteBCLNLR::componentsWithUpdates() const {
  return m_componentsWithUpdates;
}

std::vector<BCLSearchResult> RemoteBCLNLR::measuresWithUpdates() const {
  return m_measuresWithUpdates;
}

void RemoteBCLNLR::updateComponents() {
...
void RemoteBCLNLR::updateMeasures() {
...
boost::optional<BCLComponent> RemoteBCLNLR::lastComponentDownload() const {
  return m_lastComponentDownload;
}

boost::optional<BCLMeasure> RemoteBCLNLR::lastMeasureDownload() const {
  return m_lastMeasureDownload;
}

boost::optional<BCLMetaSearchResult> RemoteBCLNLR::lastMetaSearch() const {
  return m_lastMetaSearch;
}

std::vector<BCLSearchResult> RemoteBCLNLR::lastSearch() const {
  return m_lastSearch;
}

std::string RemoteBCLNLR::remoteUrl() const {
  return m_remoteUrl;
}

std::string RemoteBCLNLR::remoteProductionUrl() {
  return {REMOTE_PRODUCTION_SERVER};
}

std::string RemoteBCLNLR::remoteDevelopmentUrl() {
  return {REMOTE_DEVELOPMENT_SERVER};
}

void RemoteBCLNLR::useRemoteDevelopmentUrl() {
  m_useRemoteDevelopmentUrl = true;
  m_remoteUrl = remoteDevelopmentUrl();
  m_authKey = m_devAuthKey;
}

void RemoteBCLNLR::useRemoteProductionUrl() {
  m_useRemoteDevelopmentUrl = false;
  m_remoteUrl = remoteProductionUrl();
  m_authKey = m_prodAuthKey;
}

std::string RemoteBCLNLR::authKey() const {
  return m_authKey;
}

std::string RemoteBCLNLR::prodAuthKey() const {
  return m_prodAuthKey;
}

bool RemoteBCLNLR::setProdAuthKey(const std::string& prodAuthKey) {
...
std::string RemoteBCLNLR::devAuthKey() const {
  return m_devAuthKey;
}

bool RemoteBCLNLR::setDevAuthKey(const std::string& devAuthKey) {
...
int RemoteBCLNLR::resultsPerQuery() const {
  return m_numResultsPerQuery;
}

int RemoteBCLNLR::lastTotalResults() const {
  return m_lastTotalResults;
}

int RemoteBCLNLR::numResultPages() const {
  double numerator(lastTotalResults());
  double denominator(resultsPerQuery());
  return int(std::ceil(numerator / denominator));
}

bool RemoteBCLNLR::validateAuthKey(const std::string& authKey, const std::string& remoteUrl) {
...
boost::optional<BCLComponent> RemoteBCLNLR::waitForComponentDownload() const {
  if (waitForLock()) {
    return m_lastComponentDownload;
  }
  return boost::none;
...
bool RemoteBCLNLR::downloadComponent(const std::string& uid) {
  // Check for empty uid
  if (uid.empty()) {
...
bool RemoteBCLNLR::downloadMeasure(const std::string& uid) {
  return downloadComponent(uid);
}

bool RemoteBCLNLR::startComponentLibraryMetaSearch(const std::string& searchTerm, const std::string& componentType, const std::string& filterType) {
  // can't start another request until last one is done
  if (m_httpResponse && !m_httpResponse->is_done()) {
    return false;
  }
...
bool RemoteBCLNLR::startComponentLibraryMetaSearch(const std::string& searchTerm, const unsigned componentTypeTID, const std::string& filterType) {
  // can't start another request until last one is done
  if (m_httpResponse && !m_httpResponse->is_done()) {
    return false;
  }
...
bool RemoteBCLNLR::waitForLock() const {
...
boost::optional<RemoteQueryResponse> RemoteBCLNLR::processReply(const std::string& reply) {
...
boost::optional<BCLMetaSearchResult> RemoteBCLNLR::processMetaSearchResponse(const RemoteQueryResponse& remoteQueryResponse) {
...
std::vector<BCLSearchResult> RemoteBCLNLR::processSearchResponse(const RemoteQueryResponse& remoteQueryResponse) {
...
void RemoteBCLNLR::onDownloadComplete() {
...
int RemoteBCLNLR::setResultsPerQuery(const int numResults) {
  m_numResultsPerQuery = numResults <= 0 ? 10 : numResults > 100 ? 100 : numResults;
  return m_numResultsPerQuery;
}

void RemoteBCLNLR::setLastTotalResults(const int lastTotalResults) {
