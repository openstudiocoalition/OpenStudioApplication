...
ElectricLoadCenterDistributionController::ElectricLoadCenterDistributionController(const model::Model& model)
  : m_model(model), m_elcdView(new ELCDView()), m_elcdGridView(new GridLayoutItem()), m_gridScene(new QGraphicsScene()) {

  connect(m_elcdView->oneLevelUpButton, &QPushButton::clicked, this, &ElectricLoadCenterDistributionController::zoomOutToGridView);
...
ElectricLoadCenterDistributionController::~ElectricLoadCenterDistributionController() {
  delete m_elcdView;
  delete m_detailScene;
}

ELCDView* ElectricLoadCenterDistributionController::elcdView() const {
  return m_elcdView;
}

QSharedPointer<ELCDListController> ElectricLoadCenterDistributionController::elcdListController() const {
  return m_listController;
}

void ElectricLoadCenterDistributionController::zoomInOnELCD(const Handle& handle) {
...
void ElectricLoadCenterDistributionController::zoomOutToGridView() {
...
void ELCDListItem::zoomIn() {
  if (auto* lc = qobject_cast<ELCDListController*>(controller())) {
    lc->elcdController()->zoomInOnELCD(m_elcd.handle());
  }
}
...
ELCDListController::ELCDListController(ElectricLoadCenterDistributionController* controller) : m_elcdController(controller) {
...
ElectricLoadCenterDistributionController* ELCDListController::elcdController() const {
  return m_elcdController;
}
...
QGraphicsObject* ELCDItemDelegate::view(QSharedPointer<OSListItem> dataSource) {
  if (auto* item = qobject_cast<ELCDListItem*>(dataSource.data())) {
    ...
    connect(miniView, &ELCDSystemMiniView::zoomInClicked, qobject_cast<ELCDListController*>(item->controller())->elcdController(),
            &ElectricLoadCenterDistributionController::zoomInOnELCD);
