...
num=$(git diff $PR_BRANCH_NAME $TARGET_BRANCH_NAME --name-only --diff-filter=d | grep '.*\.\(cpp\|c\|cxx\|cxx.in\|hpp\|h\|hxx\|hxx.in\)$' | wc -l | tr -d '[:space:]')
if [ $num -eq 0 ]
then
...
git diff $PR_BRANCH_NAME $TARGET_BRANCH_NAME --name-only --diff-filter=d | grep '.*\.\(cpp\|c\|cxx\|cxx.in\|hpp\|h\|hxx\|hxx.in\)$' | xargs clang-format -style=file -i -fallback-style=none
